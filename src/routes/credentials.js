...
router.post('/authorise',
    ...
    function(req, res, next) {
        ...
        Credential.findOne({ credentialID: credentialID }, function(err, doc) {
            ...
            if (userCredentialObject === undefined) return next(errors.invalidCredentialID);
            ...
            crypto.randomBytes(128, function(err, buffer) {
                ...
                const sad = new Sad({
                    ...
                    numSignatures: Number(hashes.length)
                });
                sad.save((err) => {
                    if (err) return next(errors.databaseError);
...
router.post('/sendOTP',
    ...
    function(req, res, next) {
        ...
        Credential.findOne({ credentialID: credentialID }, function(err, doc) {
            ...
            // check access_token availability
            //Token.findOne(access_token:)
