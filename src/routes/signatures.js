...
router.post('/signHash',
   passport.authenticate('bearer', { session: false }), function (req, res, next) {
      ...
      Sad.findOne({value: utils.hash(sad), credential_id: credentialID}, function(err, sadForNumSign){
         if (err) { return next(errors.invalidSAD); }
         if (!sadForNumSign) { return next(errors.invalidSAD); }  // sau sa luam numSignatures din hashes.length
         if(sadForNumSign.numSignatures<hashes.length){
            return next(errors.invalidSAD)
         }
         else if (sadForNumSign.numSignatures!=hashes.length)
         {
            Sad.findOneAndUpdate({ value: utils.hash(sad), credential_id: credentialID}, {$inc: { numSignatures: -hashes.length}}, function (err, foundSad) {
               if (err) { return next(errors.invalidSAD); }
                  ...
                  hashes.forEach(function(itemReq,indexReq){
                     foundSad.hashes.forEach(function(item,index){ 
                        if(_.isEqual(itemReq,item)) foundHash++; 
                     })});
